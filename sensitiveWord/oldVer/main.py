...
def nmsl(words1, words2, words3, step, len, _str, result):
    if step == len:
        result.append(_str)
        return result
    else:
        nmsl(words1, words2, words3, step+1, len, _str + words1[step], result)
        nmsl(words1, words2, words3, step+1, len, _str + words2[step], result)
        nmsl(words1, words2, words3, step+1, len, _str + words3[step], result)
        return result
...
for i in range(len(words)):
    if is_chinese(words1[i][0]):
        result = nmsl(words1[i], words2[i],
                      words3[i], 0, len(words1[i]), '', [])
        for word in result:
